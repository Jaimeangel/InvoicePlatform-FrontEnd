  ...
  return (
    <div className={` ${noBorder ?'shadow border ':'' } max-w-[30rem] flex flex-col items-center mx-auto rounded-lg`}>
      ...
      <div className="w-full flex flex-col text-center">
        {
          msgError && <h1 className="font-bold italic text-2xl">Opps, algo salio mal</h1>
        }
        {
          msgError && <p className="text-justify font-semibold text-xl">{msgError}</p>
        }
        {
          !msgError && <h1 className="font-bold italic text-xl">{msg}</h1>
        }
      </div>
      {children}
    </div>
